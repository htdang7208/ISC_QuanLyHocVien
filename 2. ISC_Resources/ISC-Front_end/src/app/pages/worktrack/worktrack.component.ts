...
export class WorktrackComponent implements OnInit {
  ...
  showModal(event = null, id: number = 0) {
    ...
    if (id > 0) {
      this.worktrackService.get(id).subscribe(result => {
        ...
        if (this.worktrack.contractdate !== undefined) {
  ...
  showDeleteModal(event = null, id) {
    ...
    this.worktrack.worktrackid = id;
    this.deleteModal.show();
  }
  save() {
    if (this.worktrack.worktrackid === undefined || this.worktrack.worktrackid === 0) {
  ...
  delete(id) {
    ...
    this.worktrackService.delete(id).subscribe(result => {
      if (result.errorCode === 0) {
        const deleteCompany = this.worktracks.find(x => x.worktrackid === id);
        if (deleteCompany) {
