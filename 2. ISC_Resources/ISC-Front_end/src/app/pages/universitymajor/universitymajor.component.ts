...
export class UniversitymajorComponent implements OnInit {
  ...
  save(kind) {
    if (kind === 'university') {
      if (this.university.universityid === undefined || this.university.universityid === 0) {
      ...
      if (this.major.majorid === undefined || this.major.majorid === 0) {
  ...
  showDeleteModal(kind, event, id) {
    if (kind === 'university') {
      this.university.universityid = id;
      this.object.nativeElement.value = 'university';

    } else if (kind === 'major') {
      this.major.majorid = id;
  ...
  delete() {
    if (this.object.nativeElement.value === 'university') {
      this.universityService.deleteUniversity(this.university.universityid).subscribe(result => {
        if (result.errorCode === 0) {
          const deletedUniversity = this.universities.find( x => x.universityid === this.university.universityid);
      ...
      this.majorService.deleteMajor(this.major.majorid).subscribe(result => {
        if (result.errorCode === 0) {
          const deletedMajor = this.majors.find( x => x.majorid === this.major.majorid);
          if (deletedMajor) {
